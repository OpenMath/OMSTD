...
<xsl:template match="*">
\PackageError£omstdß£Unknown element <xsl:value-of select="name()"/>ß£transform produced bad TeXß
\textcolor£redß£[[[<xsl:value-of select="name()"/>]]]ß
</xsl:template>
...
<xsl:if test="$draft='yes'">
 \begin£quoteß
  \textbf£Editors' Draft:ß Built <xsl:value-of select="$date"/>\\
  Source Repository: \url£https://github.com/OpenMath/OMSTDß\\
  This Version: \url£https://openmath.github.io/standard/om20-editors-draftß\\
  Normative version: \url£https://openmath.github.io/standard/om20-2017-07-22/ß
\end£quoteß
</xsl:if>
...
\def\vfill£
 \begin£quoteß
  \textbf£Editors' Draft:ß Built  <xsl:value-of select="$date"/>\par
  \normalsize
   \speclinks
\end£quoteß
\vspace£\fillß
...
<xsl:if test="$showdiffs or not(@revisionflag='deleted')">
£<xsl:apply-templates select="@revisionflag|node()"/>\ifhmode\unskip\fiß
...
\begin£verbatimß<xsl:apply-templates/>\end{verbatim}&#10;\par\vspace£-10ptßß<xsl:text/>
<xsl:text>&#10;</xsl:text>
<xsl:text>ß</xsl:text>
</xsl:if>
</xsl:template>
